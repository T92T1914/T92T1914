...
Docker, Kubernetes, Prometheus and Grafana. Beats its seasonal-naive baseline by
31% (226 vs 327 MAE on 24 held-out months), and the training script exits non-zero
if it ever stops beating it, so a regression fails CI instead of shipping quietly.

**[mcts-combat-engine](https://github.com/T92T1914/mcts-combat-engine)** —
open-loop Monte Carlo Tree Search for stochastic, imperfect-information turn-based
combat. Pure Python, zero dependencies, with an original example game so every
claim in the README is runnable. Extracted and generalized from a larger private
project where the same search core evaluated 400,000+ simulations per decision
...
- **Independent reproduction over code review.** On one project every material bug
  ...
  cross-check, which only surfaced because the two answers differed.
